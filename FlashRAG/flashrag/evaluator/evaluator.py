...
class Evaluator:
    ...
    def evaluate(self, data):
        ...
        for metric in self.metrics:
            # try:
            result = self.metric_class[metric].calculate_metric(data)
            if len(result) == 2:
                metric_result, metric_scores = result
            elif len(result) == 3:
                metric_result, metric_scores, llm_results = result
                for llm_result, item in zip(llm_results, data):
                    item.update_llm_evaluation_results(metric, llm_result)
            else:
                raise ValueError("calculate_metric should return 2 or 3 values")
