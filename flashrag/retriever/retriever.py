...
class BM25Retriever(BaseTextRetriever):
    ...
    def _batch_search(self, query, num: int = None, return_score=False):
        ...
        if self.backend == "pyserini":
            ...
            results = []
            scores = []
            # 支持设置batch_size，并显示进度条
            batch_size = getattr(self, "batch_size", 64)  # 默认值64，如果有self.batch_size属性就用属性
            n = len(query)
            for start_idx in tqdm(range(0, n, batch_size), desc="Retrieving (bm25s)", ncols=80):
                batch_query = query[start_idx : start_idx + batch_size]
                query_tokens = bm25s.tokenize(batch_query)
                batch_results, batch_scores = self.searcher.retrieve(query_tokens, k=num)
                for res, sco in zip(batch_results, batch_scores):
                    results.append(list(res))
                    scores.append(list(sco))
        else:
            assert False, "Invalid bm25 backend!"
...
class DenseRetriever(BaseTextRetriever):
    ...
    def _batch_search(self, query: List[str], num: int = None, return_score=False):
        ...
        # 使用tqdm显示进度条
        for i in tqdm(range(0, len(query), batch_size), desc="Searching", unit="batch"):
            batch_query = query[i:i+batch_size]
            batch_emb = emb[i:i+batch_size]
            ...
            flat_batch_idxs = sum(batch_idxs, [])
            batch_results = load_docs(self.corpus, flat_batch_idxs)
